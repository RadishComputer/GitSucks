...
func _ready():
	...
	$Art.input_event.connect(abstract_clicked)

func abstract_clicked(viewport, event, shape_idx):
	if InputManager.click_release(event):
		if not KnowledgeManager.secretly_knows("Abstract"):
			SequenceMachine.run_sequence([
				"dialog:1061",
				"action:secretly_learn:Abstract"
			], self)
		else:
			SequenceMachine.run_sequence(["dialog:1066"], self)
