...
func _ready():
	...
	$Art.input_event.connect(babys_breath_clicked)

func babys_breath_clicked(viewport, event, shape_idx):
	if InputManager.click_release(event):
		if not KnowledgeManager.secretly_knows("Babys_Breath"):
			SequenceMachine.run_sequence([
				"dialog:1071",
				"action:secretly_learn:Babys_Breath",
			], self)
		else:
			SequenceMachine.run_sequence([
				"dialog:1072",
				"action:secretly_forget:Babys_Breath",
			], self)
