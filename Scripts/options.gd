...
func _ready():
	...
	$VBox/HBox1/Switch.pressed.connect(on_item_mode_pressed.bind(SettingsManager.ItemMode.SWITCH))
	$VBox/HBox1/Hold.pressed.connect(on_item_mode_pressed.bind(SettingsManager.ItemMode.HOLD))
	update_item_mode_visual()

	$VBox/HBox2/Plain.pressed.connect(on_menu_style_pressed.bind(SettingsManager.MenuStyle.PLAIN))
	$VBox/HBox2/Pure.pressed.connect(on_menu_style_pressed.bind(SettingsManager.MenuStyle.PURE))
...
func on_item_mode_pressed(mode: int):
	SettingsManager.item_mode = mode
	SettingsManager.save_settings()
	update_item_mode_visual()
	
func update_item_mode_visual():
	var is_switch = SettingsManager.item_mode == SettingsManager.ItemMode.SWITCH
...
func on_menu_style_pressed(mode: int):
	SettingsManager.menu_style = mode
	SettingsManager.save_settings()
	update_menu_style_visual()
	update_high_contrast_visual()
	update_grayscale_visual()
...
func update_menu_style_visual():
	var is_plain = SettingsManager.menu_style == SettingsManager.MenuStyle.PLAIN
...
func on_high_contrast_pressed(value: bool):
	SettingsManager.high_contrast = value
	SettingsManager.save_settings()
	update_high_contrast_visual()

func update_high_contrast_visual():
	var enabled = SettingsManager.high_contrast
...
func on_grayscale_pressed(value: bool):
	SettingsManager.grayscale = value
	SettingsManager.save_settings()
	update_grayscale_visual()

func update_grayscale_visual():
	var enabled = SettingsManager.grayscale
...
func on_bounce_pressed(value: bool):
	SettingsManager.bounce_mode = value
	SettingsManager.save_settings()
	update_bounce_visual()

func update_bounce_visual():
	var enabled = SettingsManager.bounce_mode
...
func on_size_pressed(size: String):
	SettingsManager.text_size = size
	SettingsManager.save_settings()
	update_size_visual()
	SettingsManager.apply_text_theme()

func update_size_visual():
	var size = SettingsManager.text_size
...
func on_display_pressed(index: int):
	SettingsManager.set_preset(index, false)
	update_resolution_visual()
...
func update_resolution_visual():
	for i in range(SettingsManager.PRESETS.size()):
		var path = "VBox/HBox6/Display%d" % i
		if has_node(path):
			get_node(path).button_pressed = false
	
	if not SettingsManager.use_custom_resolution:
		var idx = clamp(SettingsManager.preset_index, 0, SettingsManager.PRESETS.size() - 1)
	...
	if has_node("VBox/HBox6/Full"):
		$VBox/HBox6/Full.button_pressed = SettingsManager.fullscreen
