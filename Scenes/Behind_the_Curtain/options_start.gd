...
func update_high_contrast_visual():
	...
	if color_rect:
		color_rect.material.set_shader_parameter("high_contrast_enabled", enabled)
...
func update_grayscale_visual():
	...
	if color_rect:
		color_rect.material.set_shader_parameter("grayscale_enabled", enabled)
...
func update_resolution_visual():
	for i in range(GameGlue.SettingsManager.PRESETS.size()):
		var path = "VBox/HBoxD/Display%d" % i
		if has_node(path):
	...
	if not GameGlue.SettingsManager.use_custom_resolution:
		...
		var path = "VBox/HBoxD/Display%d" % idx
		if has_node(path):
			get_node(path).button_pressed = true

	if has_node("VBox/HBoxD/Full"):
		$VBox/HBoxD/Full.button_pressed = GameGlue.SettingsManager.fullscreen
