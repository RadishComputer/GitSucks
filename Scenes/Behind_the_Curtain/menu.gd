...
func set_music(open: bool) -> void:
	if open:
		if not music_started:
			GameGlue.AmbiencePlayer.stream = preload("res://Sounds/Salmon_Run_Muted.wav")
			GameGlue.AmbiencePlayer.process_mode = Node.PROCESS_MODE_ALWAYS
			GameGlue.AmbiencePlayer.max_polyphony = 1
			GameGlue.AmbiencePlayer.play()
			music_started = true
		GameGlue.AmbiencePlayer.stream_paused = false
	else:
		GameGlue.AmbiencePlayer.stream_paused = true
